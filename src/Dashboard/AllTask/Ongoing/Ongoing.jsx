...
const Ongoing = ({ reload }) => {
    ...
    const ongoingTasks = data.filter((task) => task.status.toLowerCase() === 'ongoing');

    return (
        <div>
            <h1 className="text-2xl bg-gray-200">Ongoing</h1>
            {loading && <p>Loading...</p>}
            {!loading && ongoingTasks.length === 0 && <p>No Ongoing tasks found.</p>}
            {!loading &&
